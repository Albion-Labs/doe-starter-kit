...
6. **Work on feature branches, commit per step.** Each feature gets its own branch (`feature/feature-name`), created at session start by `/crack-on`. Commit after every completed step on the feature branch — never batch multiple steps. Push each commit to the remote immediately. Do not commit directly to main. At feature completion (retro step), create a Pull Request to main using `gh pr create` with the PR template auto-filled from contract criteria. CI Result must pass before merge (DOE's universal CI auto-detects your stack and runs the right checks). Do not add "Co-authored-by" trailers or any AI attribution to commit messages. This gives every feature a reviewable PR history and keeps main stable.
...
- Building any feature that handles personal data (user accounts, CRM, canvassing, donations, membership, surveys, electoral register) → read `directives/data-compliance.md (if exists)` BEFORE writing any code. DPIA is a hard blocker.
- Building email, SMS, or communications features → read `legal-framework.md (if exists)` ## PECR section. Political party exemption is narrow — cold-contacting non-members requires consent.
