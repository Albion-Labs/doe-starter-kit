...
6. **Work on feature branches, commit per step.** Each feature gets its own branch (`feature/feature-name`), created at session start by `/crack-on`. Commit after every completed step on the feature branch — never batch multiple steps. Push each commit to the remote immediately. Do not commit directly to main. At feature completion (retro step), create a Pull Request to main using `gh pr create` with the PR template auto-filled from contract criteria. CI (ESLint, Playwright, Lighthouse) must pass before merge. Do not add "Co-authored-by" trailers or any AI attribution to commit messages. This gives every feature a reviewable PR history and keeps main stable.
