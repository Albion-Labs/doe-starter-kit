...
The companion `block_dangerous_commands` hook splits its match logic in v1.60.1: substring patterns (`DROP TABLE`, fork bombs, `dd if=`, etc.) keep substring-match semantics; env-var bypass names (`SKIP_REVIEW_GATE`, `SKIP_CONTRACT_CHECK`, `SKIP_SIGNOFF_CHECK`, `BYPASS_BLOCK` as of v1.65.1) match only on actual assignment (`VAR=`), with a quote-context heuristic that skips the match when it sits inside a quoted string. Reading docs, running `grep`, or echoing the flag name no longer trip a false-positive block. Recursive-force `rm` is detected by a dedicated token check (flag-order-, whitespace- and quote-insensitive), so `rm -fr /`, `rm -r -f /` and `rm -rf "/"` are all caught while a specific path like `rm -rf ./build` is not; the `.delete()` / `deleteMany()` substrings were removed because they false-positived on ordinary ORM calls. Treat the hook as accident-prevention, not a security boundary — a target reached via a shell variable still slips through by design.

`block_unnecessary_admin_merge` (added v1.65.1) intercepts `gh pr merge --admin` and queries the authoritative `mergeStateStatus` before allowing the override. Catches a recurring AI failure mode where the immediate-post-creation `UNKNOWN` window (GitHub still registering check-runs from the workflow, ~30-60s) is misread as a permanent block. Refuses if state is `CLEAN` (admin unnecessary) or `UNKNOWN` (transient), surfaces failing check-runs and requires `BYPASS_BLOCK=1` if state is `BLOCKED`, fails closed on API errors, passes through for `DIRTY`/`BEHIND`/non-`OPEN` states. Operates downstream of `confirm_pr_merge` so the bordered-card flow fires first.
...
- **Pending-PR sync, contract verification, quality-gate checkpoints.** Bigger checks that fire when the relevant files are staged. Skip the pending-PR sync check with `SKIP_PENDING_PR_CHECK=1` (e.g. when committing while a known PR is intentionally still open).
