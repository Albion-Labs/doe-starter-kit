...
<!-- /hero -->
<!-- background -->
This is the first big cut of v2.0 Workstream 1/2, taken docs-first. What went with the pages: the ~20-file stamping tax on every release (`stamp_tutorial_version.py` + the auto-release stamp step + `chore(stamp)` commits), the whats-new regen step and its release-time commit, the pre-push tutorial-docs-version gate (which repeatedly false-blocked releases — see the v1.58.0 and session-220 retros), the pre-push whats-new tag-freshness gate, and the entire pre-commit doc-freshness block. Release ceremony after this change: detect CHANGELOG version → tag → push tag → `gh release create` with notes lifted from the CHANGELOG entry — no release-time commits at all. Issue #35 (auto doc sync) is mooted by this release: docs that don't exist can't go stale.
<!-- /background -->

### Removed
- **docs/tutorial/** — the entire directory: all 18 tutorial pages, `hooks.md`, `kit-version.js`, `whats-new.html`, and the favicon assets.
- **execution/stamp_tutorial_version.py**, **execution/generate_whats_new.py**, **tests/execution/test_generate_whats_new.py** — the doc-generation/stamping layer and its tests.
- **.github/workflows/auto-release.yml** — the whats-new regen, stamp, commit, and push-commit steps (plus the now-unneeded Python setup and git-identity steps). Releases no longer touch the tree.
- **.githooks/pre-push** — both docs gates (tutorial-docs-version and whats-new tag-freshness). The methodology quick check is the hook's one remaining job.
- **.githooks/pre-commit** — the doc-freshness warning block entirely (all five mappings and the `SKIP_DOC_FRESHNESS` escape).
- **CLAUDE.md** — the "Updating CHANGELOG.md → regenerate whats-new" trigger.
- **directives/starter-kit-sync.md** — the post-sync tutorial checklist and the 7-row "check this tutorial page" table; **directives/kit-development.md** — the stamp/regen steps in the manual release fallback.

### Changed
- **.githooks/commit-msg** — the version-tag changelog enforcement now requires `CHANGELOG.md` staged (was `whats-new.html`): the gate checks the actual release record instead of the derived page.
...
- **README.md / CUSTOMIZATION.md / .github/ISSUE_TEMPLATE/config.yml / global-commands/sync-doe.md / directives/testing-strategy.md** — point at `docs/reference/` and CHANGELOG.md-as-release-record; README execution-script count 20 → 18 (caught by `readme_claims_match_disk`).
- **tests/githooks/test_pre_push.py** — rewritten: regression tests that both retired gates stay gone, plus a methodology-failure-blocks test. Fixture fix: the hook is copied into the fixture repo (it derives PROJECT_ROOT from its own location; the old tests ran it in-place against the real kit's files).
- **tests/githooks/test_pre_commit.py** — doc-freshness tests replaced by a single regression test that the retired check stays silent.
...
### Added
- **`.claude/hooks/stamp_todo_timestamps.py`** -- new PostToolUse hook (~115 lines). Detects `[x]` step lines in `tasks/todo.md` lacking `*(completed HH:MM DD/MM/YY)*` and appends the stamp inline. Idempotent: already-stamped lines (and lines with the older date-only stamp form `*(completed DD/MM/YY)*`) are skipped. Anchors to `$CLAUDE_PROJECT_DIR` per the v1.62.2 cwd-safety pattern, so the hook resolves the real `tasks/todo.md` even after shell cwd drift. Mirrors `audit_claims.py`'s `_TASK_DONE_RE` and `_TIMESTAMP_RE` exactly so stamps satisfy the audit byte-for-byte. Contract criteria (`- [x] [auto] foo`) are skipped because they're dash-prefixed and don't match the audit's task regex. Triggers on `Edit | Write | MultiEdit` and stays silent on non-todo files and when no stamp is needed. PostToolUse failures never block tools — on any unexpected error (bad JSON, IO error) the hook silently returns.
- **`.claude/hooks/stamp_todo_timestamps.py --pre-commit FILE`** -- second invocation mode. Stamps the named file in-place, exits 0. Called by `.githooks/pre-commit` to catch direct human edits to `tasks/todo.md` that bypassed the PostToolUse hook (no Claude session involved). Same regexes, same idempotence, same silent-on-error.
- **`tests/claude_hooks/test_stamp_todo_timestamps.py`** -- 17 tests covering pure `process()` (regex behaviour, idempotence on already-stamped lines, date-only stamp tolerance, contract-criteria exemption, indented-unnumbered tasks, retro steps, multi-line input), the JSON-on-stdin subprocess path, repeated-firing convergence, the `--pre-commit` CLI mode, missing-file safety, and `$CLAUDE_PROJECT_DIR` resolution under stale-path conditions.

### Fixed
- **`.githooks/pre-commit`** -- new 11-line auto-stamp block that runs before the existing `audit_claims.py --hook` invocation. Finds staged `tasks/todo.md` files via `git diff --cached --name-only --diff-filter=ACM`, runs `stamp_todo_timestamps.py --pre-commit` on each, re-stages with `git add`. Skipped silently when `.claude/hooks/stamp_todo_timestamps.py` is missing (older project that hasn't pulled v1.64.0 yet). Every existing pre-commit gate (`SKIP_MAIN_PROTECTION`, ESLint, stub detection, audit, signoff, contract verification, pending-PRs sync, test freshness, doc freshness) untouched. No new `SKIP_*` flag — the auto-stamp step is purely additive and never blocks.
- **`.claude/settings.json`** -- registered `stamp_todo_timestamps.py` on the existing `Edit | Write | MultiEdit` PostToolUse matcher, joining `copy_plan_to_project.py` and `check_completed_feature.py`. Hook command uses the `$CLAUDE_PROJECT_DIR` form established in v1.62.2.

### Pull impact
**No migration step required.** Project-side `setup.sh` hook-merge deduplicates by exact command-string match — the new `stamp_todo_timestamps.py` entry is a new hook (not a modification of an existing one), so it merges cleanly into project `settings.json` on `/pull-doe`. After the pull, active Claude Code sessions in the project must restart so `.claude/settings.json` is re-read and the new hook starts firing. Existing `tasks/todo.md` content is untouched until the next `[x]` flip on a freshly-edited line, at which point stamping happens silently on the agent's next file write.
...
## v1.63.0 (2026-05-12)
<!-- hero -->
Worktree convention + parallel-session auto-detection lands the structural fix for branch-level races between concurrent Claude Code sessions on the same project. The convention itself (`<project>/` on the default branch, feature work in `<project>-<feature>/` siblings) shipped early as v1.61.5; v1.63.0 wires the convention into the kit's daily loop so the AI surfaces it at the right moments. `/stand-up` now reads `git worktree list --porcelain` and shows a WORKTREES row when 2+ worktrees are present, with a `!! Detached:` line for any orphaned debug worktree and an honest-scope footnote: worktrees fix BRANCH-level races (HEAD-stomping between sessions), not FILE-level edit conflicts on shared docs (STATE.md, todo.md, learnings.md, CLAUDE.md still need single-terminal coordination). `/wrap` detects branch drift via `.tmp/.session-start-branch` (written by `/crack-on` at kick-off) and auto-switches to the trunk worktree for bookkeeping commits so stats land on main regardless of where the feature work happened. `/sync-doe` and `/pull-doe` gain a trunk-worktree pre-flight using `git symbolic-ref refs/remotes/origin/HEAD` (resilient across main-default and legacy master-default repos). Two new slash commands `/worktree-create` and `/worktree-remove` operationalise the convention with the safety gates the Step 0 spike found necessary: locked worktrees stay locked until explicit `git worktree unlock`, trunk worktrees stay non-removable. The wizard surfaces a "Working with parallel sessions later?" hint at init time so first-time users discover `/worktree-create` before they need it.

Step 0 was a real spike (`.tmp/worktree-spike.md` in a consumer project) that validated four areas with evidence: CWD detection in PostToolUse hooks, per-worktree firing, shared `.tmp/` semantics, and `git worktree list --porcelain` parsing across normal / detached / locked / bare records. The spike caught three pre-existing relative-path landmines in hook scripts (silent false-pass under shell cwd drift, the same failure class v1.62.2 fixed at the invocation layer) and bundled the fixes into Step 2 rather than ship a separate v1.62.3 patch. The kit test suite grew from 199 to 203 tests across this release (+7 from Step 2's hook regression tests, +4 from Step 5's wizard-hint tests, with deduplication against existing hook tests).
<!-- /hero -->

### Added
- **`global-commands/worktree-create.md`** -- new slash command (~50 lines). Takes a feature-slug argument, validates kebab-case form, runs the trunk-worktree pre-flight (default-branch lookup + branch / path collision checks), then creates branch + sibling worktree in one `git worktree add ... -b feature/<slug>` operation. Post-create card surfaces the new path, branch name, and a hint to open Claude Code in the sibling for parallel session work. Edge cases documented: detached HEAD on trunk, uncommitted changes on trunk, bare repository, path-with-spaces.
- **`global-commands/worktree-remove.md`** -- new slash command (~80 lines). Takes a feature-slug OR path argument; resolves either form. Six pre-flight gates in order: argument resolution, worktree-existence check, trunk refusal (cannot remove the worktree holding the default branch), **locked-worktree check (spike Goal 4 finding)** -- surfaces the lock reason verbatim and requires `git worktree unlock` before re-run, cleanliness check (uncommitted changes -> ask user), unmerged-branch check (branch not in `--merged` -> ask). After removal, offers branch cleanup as a separate step (safe `-d` for merged, user-confirmed `-D` for unmerged). `--force` stays unexposed -- users who genuinely need it run `git worktree remove --force` directly.
- **`tests/claude_hooks/test_hook_paths_cwd_safe.py`** -- 7 regression tests for the Step 2 hook landmine fixes. Subprocess-driven, foreign cwd. Uses `Path(__file__).resolve().parents[2]` to resolve the kit root dynamically so the test runs against the local checkout (worktree or main) -- pre-merge worktree verification was the motivating use case.
- **`tests/execution/test_init_worktree_hint.py`** -- 4 test cases asserting the wizard's worktree hint surfaces across solo + team modes, references both the slash command and the convention directive, and is independent of `is_empty` (fresh vs existing project).
- **`/stand-up` WORKTREES row (kick-off + status cards)** -- runs `git worktree list --porcelain`, shows count + branches when 2+ records exist, surfaces detached worktrees with a "no branch checked out" hint, and appends the honest-scope footnote so the BRANCH-vs-FILE distinction stays front of mind.
- **`/wrap` Step 0a worktree handling** -- branch-drift detection via `.tmp/.session-start-branch` plus trunk-worktree auto-switch for bookkeeping commits. Surfaces drift instead of silently continuing.
- **`/crack-on` session-start branch capture** -- `git branch --show-current > .tmp/.session-start-branch` on every kick-off. Shell `>` redirect replaces prior contents each invocation; `/wrap` consumes this file for drift detection.
- **`/sync-doe` Step 0.1 trunk-worktree pre-flight** -- verifies `~/doe-starter-kit`'s main worktree is on the default branch before any kit operation. Trunk identified via `git symbolic-ref refs/remotes/origin/HEAD`.
- **`/pull-doe` pre-flight section** -- verifies the project's current worktree is on the default branch before pulling.
- **`directives/delivery-rules.md` plan-refresh worktree note** -- standalone chore PRs from plan refreshes run from the trunk worktree on `chore/plan-refresh-*` branches. Same trunk-identification heuristic.
- **`execution/doe_init.py` worktree hint in `card_done()`** -- 4-line block surfaces "Working with parallel sessions later?" + `/worktree-create <feature-slug>` + directive reference. Universal across solo / team / regulated modes.

### Fixed
- **`.claude/hooks/check_completed_feature.py`** -- anchors `tasks/todo.md` to `$CLAUDE_PROJECT_DIR` instead of relative `Path("tasks/todo.md")`. Pre-fix the hook silently no-op'd under shell cwd drift -- the "all steps complete, move feature to Done" warning vanished. The hook returned cleanly with no warning, indistinguishable from "no completion to warn about", which is the dangerous failure mode (lost safety net).
- **`.claude/hooks/enforce_review_gate.py`** -- anchors three paths to `$CLAUDE_PROJECT_DIR` via a new `_project_root()` helper: (1) `tasks/todo.md` in `check_steps_complete` (pre-fix returned `(True, "")` on missing file, silently false-passing the PR gate -- same class as the v1.57.0 pipeline silent-pass bug), (2) `.tmp/review-passed-<branch>.json` artifact path (pre-fix wrote / read against cwd/.tmp, missing the artifact and blocking with a misleading "review required" error after shell drift), (3) `git branch / rev-parse` invocations now use `git -C <project_root>` so they work from any cwd, not just from inside the project tree.

### Pull impact
**New slash commands install automatically via `setup.sh`.** Running `/pull-doe` after this release applies `/worktree-create` and `/worktree-remove` to `~/.claude/commands/` for global use, plus the updated `/stand-up`, `/wrap`, `/crack-on`, `/sync-doe`, `/pull-doe` definitions. The hook fixes in `.claude/hooks/` flow through `setup.sh` hook-merge (which deduplicates by exact command-string match -- the hook script bodies themselves change, but the settings.json command paths stay the same as v1.62.2's `$CLAUDE_PROJECT_DIR` form, so no manual rewrite is needed this round).

**`/worktree-create` defaults:** sibling path is `<dirname of current worktree>/<basename>-<slug>`, branch name is `feature/<slug>`. Run it from the trunk worktree.

**Worktree convention is opt-in.** Single-session projects ignore the pattern (one folder, branch in HEAD as today). Multi-session projects opt in by creating worktrees per long-lived branch. The new `/stand-up` WORKTREES row omits itself when only one worktree exists, so single-session users see no change.

**Honest scope (preserved in three places now):** worktrees fix BRANCH-level races, not FILE-level. Shared docs (STATE.md, todo.md, learnings.md, CLAUDE.md) still need single-terminal coordination at the file level even when the branch is locked down. This is surfaced in: the `/stand-up` WORKTREES footnote, the `/wrap` Step 0a honest-scope reminder, and the `/worktree-create` post-create card.

---

## v1.62.2 (2026-05-12)
<!-- hero -->
Safety-critical fix for a latent footgun in every kit install since hooks landed: the kit's `.claude/settings.json` template registered every PreToolUse / PostToolUse hook with a **relative** command path (`python3 .claude/hooks/X.py`). Claude Code's persistent Bash shell preserves cwd across tool calls, so any legitimate `cd` (spinning up a local dev server, running a script from a subdirectory) permanently changed the shell's working directory. Subsequent tool calls fired the hooks, which tried to resolve the relative script path against the new cwd, hit errno 2 file-not-found, exited non-zero, and Claude Code interpreted the non-zero exit as **the hook blocking the tool**. Net effect: a single legitimate `cd` bricked every subsequent Edit / Write / Bash tool call in the session. Only Read still worked (PostToolUse failures are informational). Escape required `/clear`, losing the conversation context. Hit during a routine dev workflow on a consumer project. Repeats across every project that ran `setup.sh` against kit v1.62.1 or earlier.
...
Two supporting docs/code fixes round out the release. **Workflow framing aligned** across `universal-claude-md-template.md`, `SYSTEM-MAP.md`, and `docs/tutorial/new-project.html` — the pre-v1.60.0 "make changes in project repo first; run /sync-doe at feature end" phrasing is replaced with "two ways to land a kit change, both PR-based" framing. **`wrap_stats.py` first-session bug fixed** — `_merge_main_streak` raised `TypeError: '>' not supported between NoneType and NoneType` on every new DOE project's first `/wrap` because `dict.get(key, default)` returns the actual `None` (not the default) when the key exists with a null value. Six regression tests added.

**Version bump rationale**: minor for the `/sync-doe` feat. The worktree auto-detection layer previously queued for v1.62.0 (per v1.61.5's footer) is rebumped to **v1.63.0** in the release queue. Source: a consumer project's bootstrap session audit — the slash command's "release directly on main" Step 12 was caught contradicting the canonical directive's "opens the kit PR" framing.
<!-- /hero -->

### Added
- **Step 0.7 in `directives/starter-kit-sync.md`** — `/sync-doe` resume detection. On every invocation, checks for `~/doe-starter-kit/.tmp/.sync-doe-pending-release.json`. If the state file exists, branches on `gh pr view`'s reported state: `MERGED` jumps to Phase 2; `OPEN` stops with "merge it first"; `CLOSED` (without merge) prompts to discard pending state. Conversational shortcut: replying "merged" / "yes" / "done" routes to Phase 2 cross-session and cross-project (state file lives in the kit, accessible from anywhere).
- **Step 11 in `directives/starter-kit-sync.md`** — Phase 2: release after merge. Pre-flight `gh` exit-code guard separates transient `gh` failures from "PR not merged". Idempotency checks (`git rev-parse "$VERSION"` + `gh release view "$VERSION"`) gate the stamp + commit + tag + push and `gh release create` blocks — Phase 2 is now safely re-runnable after partial failures (e.g., `gh release create` flakes after the tag was pushed). Tag-then-commit-push order preserved per the universal pre-push tutorial-docs-version gate learning.
- **6 regression tests in `tests/execution/test_wrap_stats.py`** — covering `_merge_main_streak` with both null `lastSessionDate` (the original bug — was `TypeError`, now no-op), main newer than local (copies), local newer than main (does not copy), null main + dated local (does not copy), git fetch failure (returns data unchanged), malformed JSON from origin (caught, returns data). All 21 wrap_stats tests pass.

### Changed
- **`directives/starter-kit-sync.md` Step 10** rewritten — Phase 1 procedure replaces the pre-v1.60.0 "stamp + commit + tag + push + release directly on main" with "branch + commit + push + open PR + write state file + stop". Branch name is sanitized via `tr` against the project name; the PR body is a single-quoted heredoc (Claude substitutes placeholders); state-file write is pure-Bash (un-quoted heredoc, no Python heredoc env-var ceremony); explicit `gh pr view` for PR_URL/PR_NUMBER capture.
- **`global-commands/sync-doe.md`** rewritten to mirror the directive's two-phase structure. Adds new Result Summary states: `🟡 PR_OPENED`, `✅ RELEASED`, `🟡 AWAITING_MERGE`, `❌ PR_CLOSED`. Drops references to the retired `.tmp/.sync-doe-active` flag file (the v1.60.0 `guard_kit_writes` no longer reads it). Drops `SKIP_STEP_MARK_CHECK=1` from Phase 1 commits (the `sync:` prefix doesn't trigger the step-mark hook). Drops the cross-project-ambiguous Phase 2 STATE.md update.
- **`universal-claude-md-template.md`** ## DOE Starter Kit bullet — replaces "make changes in project repo first; run /sync-doe at feature end" with "Two ways to land a kit change, both PR-based: (1) Manual PR for one-off; (2) /sync-doe for bundling project-originated changes into a release — opens a PR (Phase 1), then runs post-merge release machinery (Phase 2) with narrowly-scoped SKIP_MAIN_PROTECTION".
- **`SYSTEM-MAP.md`** `guard_kit_writes.py` row — corrected to v1.60.0 reality: defence-in-depth blocking only irreversible Bash ops (recursive removal, force-push to kit main); not a file-edit gate.
- **`docs/tutorial/new-project.html`** — scoped the "kit is never modified" reassurance to setup.sh specifically; dropped the kit-maintainer-workflow parenthetical (noise for the new-user audience).

### Fixed
- **`execution/wrap_stats.py` `_merge_main_streak`** — null-coerce `lastSessionDate` via `or ""` so first-session wraps no longer hit `None > None` TypeError. Discovered: every new DOE project's first `/wrap` immediately after `bash setup.sh`. Regression covered by 6 new pytest cases.

### Pull impact
**Authored migration manifest**: `migrations/v1.62.0.md`. Tier 1 phrase rewrites in `universal-claude-md-template.md`, `SYSTEM-MAP.md`, and `docs/tutorial/new-project.html`. Tier 2 (directives) rewrites in `directives/starter-kit-sync.md`. Behavioural change: `/sync-doe` flow is now two-phase. Projects on v1.61.5 pulling to v1.62.0 should re-read the migration manifest before their next sync — the procedure they're about to follow has changed substantively. The slash command (`global-commands/sync-doe.md`) is the authoritative quick-reference; the directive (`directives/starter-kit-sync.md`) has the full step-by-step.

---

## v1.61.5 (2026-05-08)
<!-- hero -->
Adds the worktree convention to the kit so every DOE project inherits the structural fix for parallel-session branch races. New `directives/parallel-worktrees.md` covers when to use, setup, scope (worktrees fix the BRANCH-level race -- silent HEAD switch when one session's `git checkout` moves another session's HEAD -- but do NOT fix the FILE-level race for shared docs like STATE.md, todo.md, learnings.md, CLAUDE.md, which still need "edit from one terminal at a time" coordination), and cleanup. Universal CLAUDE.md template gains a `## Parallel Sessions` section stating the convention as a positive-form rule. Kit's CLAUDE.md template gains a new trigger pointing to the directive; the existing parallel-sessions trigger (which actually covered wave/DAG dispatch) is renamed for clarity. This is the convention-statement patch -- the auto-detection layer (drift detection in `/stand-up` and `/wrap`, pre-flight checks in kit-write commands, `/worktree-create` and `/worktree-remove` slash commands, `/crack-on` hook) is queued separately as v1.62.0. Source: a consumer project's session retro -- the parallel-session race silently moved wrap commits onto a long-lived feature branch.
<!-- /hero -->
...
- **`global-commands/crack-on.md`, `stand-up.md`, `wrap.md` -- `KIT DIRTY` row / wrap-output line** -- session-boundary detection points for the v1.60.0 PR-only model's residual exposure. When `cd ~/doe-starter-kit && git status --porcelain` returns non-empty, surface the count + first 3 paths in the kick-off card (`/crack-on`), morning briefing (`/stand-up`), or wrap output (`/wrap`). Advisory only -- never blocks. Closes the temporal gap between a silent kit-working-tree edit and the next `/sync-doe` Step 0.5 catch.
...
## v1.59.1 (2026-04-29)
<!-- hero -->
Hook hygiene patch closing two LOW findings from the v1.59.0 consumer-project-side `/review`: `protect_directives.py` allows new directive files via an existence check, and `guard_kit_writes.py` tightens its redirect/`tee` patterns to eliminate heredoc-body false positives. Adds a stop-gap cwd-aware early-return to `guard_kit_writes.py` ahead of the v1.60.0 PR-only refactor.
<!-- /hero -->
...
## v1.59.0 (2026-04-29)
<!-- hero -->
Pink Elephant rewrite: load-bearing prohibitions across the kit's prompt surface (`Never X`, `Don't Y`, `DO NOT Z`) rewritten to positive `When X, do Y` form per the Pink Elephant article (arxiv 2503.22395), with a `Tradeoff:` line added to 25 of 26 flat directives. Also fixes a silently-broken hook matcher: `.claude/settings.json` PreToolUse and PostToolUse entries shipped with lowercase patterns that never matched Claude Code's actual Tool name strings, so every kit hook had been quietly non-functional since introduction. New `migrations/v1.59.0.md` manifest format documents 150 OLD/NEW phrase pairs across 33 files plus four behavioural-change records, consumed by a new `/pull-doe` Step 4.5 pre-flight that surfaces a PULL IMPACT panel before any change is applied.
<!-- /hero -->
...
## v1.58.0 (2026-04-27)
<!-- hero -->
Tested code and tutorial docs gain freshness guardrails. Two warning-only blocks added to `.githooks/pre-commit` flag commits where a tested source file is staged without its sibling test, and where source files ship without their tutorial-doc counterpart. Both warnings name the missing path so the fix is one ack-and-add away; neither blocks the commit -- they're nudges, not gates.
<!-- /hero -->
...
## v1.57.0 (2026-04-27)
<!-- hero -->
Phase 1 of Conventional Commits across the DOE kit: two new directives, an extended `commit-msg` hook, a CLAUDE.md template section, and five command/directive migrations bundled so that downstream projects bootstrap with the convention from session zero. The new `commit-msg` validator runs in `warn` mode by default during the v1.57 -> v1.58 transition window -- non-compliant subjects print a stderr warning but the commit still succeeds; set `DOE_COMMIT_HOOK_MODE=block` once your team is fully on the convention. Phase 2 (auto-changelog generator, hook block-mode default, PR-title GitHub Action) deferred to a future session.
<!-- /hero -->
...
## v1.56.1 (2026-04-27)
<!-- hero -->
Patch release fixing three spec deviations from v1.56.0 against kit issues #15 and #16. The auto-commit message now stamps the kit release that produced it (`chore: initial DOE scaffolding (kit vX.Y.Z)`); the `.env` bootstrap refuses to create the secrets file unless `.gitignore` actively excludes `.env`; and the bootstrap confirmation guides users to fill in `.env` values before running anything. End-to-end wizard runs verified both positive and negative paths.
<!-- /hero -->
...
## v1.56.0 (2026-04-27)
<!-- hero -->
Bootstrap polish: collapses four manual post-setup steps into opt-in wizard prompts so `bash ~/doe-starter-kit/setup.sh` leaves a fresh project ready for `gh repo create` with no workarounds. The wizard now offers a `chore: initial DOE scaffolding` commit on main (pre-checks `user.email`), copies `.env.example` to `.env` for local dev, normalises the project branch to `main` ahead of the scaffolding commit, and the pre-commit main-branch protection allows the first-ever commit on a fresh repo. Resolves kit issue #19, supersedes #15, #16, #18.
<!-- /hero -->
...
### Fixed
- **building-rules.md: project-specific content** -- removed `execution/build.py` trigger (project-specific), replaced domain-specific examples with generic equivalents.
...
- **data-safety.md: project-specific content** — replaced the project name with "the system" and removed 2 project-specific `.claude/plans/` cross-references that don't exist in other projects.
