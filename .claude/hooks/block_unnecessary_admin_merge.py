...
def _query_pr_state(pr_number: str):
...
def _failing_checks(sha: str) -> str:
...
def main() -> None:
    ...
    pr_state, mss, sha, err = _query_pr_state(pr_number)
    if pr_state is None:
        _emit_block(
    ...
    if mss == "BLOCKED":
        failing = _failing_checks(sha) if sha else "(no SHA available)"
        _emit_block(
