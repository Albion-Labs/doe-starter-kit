...
No external resources (offline, nothing to leak). Reads ONLY the v1.0 schema —
the same contract HQ consumes.
...
.hero{display:flex;gap:34px;padding:28px 26px;align-items:center;flex-wrap:wrap}
.big{font-size:62px;font-weight:650;line-height:1;letter-spacing:-.02em;color:var(--ink)}
.big small{font-size:20px;color:var(--mut);font-weight:500}
.stat{display:flex;flex-direction:column;gap:4px}
.stat .n{font-size:21px;font-weight:600}
.stat .l{font-size:10.5px;letter-spacing:.07em;text-transform:uppercase;color:var(--dim)}
...
def render(sc):
    ...
    for g in by:
        ...
        label = "CAUGHT" if ok else "MISS"
        note = "" if ok else "no deterministic gate &mdash; needs a test/contract"
        rows.append(
    ...
    hero = (
        ...
        f'<div class="stat"><div class="n mono">{ctrl["caught"]}/{ctrl["injected"]}</div>'
        '<div class="l">caught without DOE</div></div>'
    ...
    foot = (
        '<div class="foot">'
        '<b>Control = 0 is the real counterfactual, not a trick.</b> Vanilla tooling '
        '(raw Claude, Lovable, Replit) has none of these gates, so it catches none of '
        'these defects.<br>'
        '<b>Why 86%, not 100%?</b> One planted defect is a behavioural logic bug no '
        'static gate can catch &mdash; shown as an honest miss so you see exactly where '
        'DOE&rsquo;s gates stop and a human test is still needed. Every class DOE claims '
        'to cover was caught by its actual, unmodified gate script.<br>'
