...
12. **Verify PR merged**, pull main, regen whats-new + commit + tag + push + release:
    ...
    - If `TAG_EXISTS=0`: `python3 execution/generate_whats_new.py` → `SKIP_MAIN_PROTECTION=1 SKIP_STEP_MARK_CHECK=1 git commit -am "chore(release): regen whats-new for $VERSION"` → `git tag "$VERSION"` → `git push origin "$VERSION"` (tag first) → `SKIP_MAIN_PROTECTION=1 git push` (then commit)
