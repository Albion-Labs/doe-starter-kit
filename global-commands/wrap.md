...
6. **DOE Kit sync check** — If `~/doe-starter-kit` exists:
   ...
   - **Sync gap check:** Run `python3 execution/audit_sync.py --json 2>/dev/null` and count `missing_from_kit` entries. If > 0, show a warning in the wrap output: `Sync gap: N universal file(s) not in kit. Run /sync-doe.` Record count in doeKit data (e.g. `"syncGap": 3`). If the script doesn't exist or fails, skip silently.
