...
After the codebase roast, add an **"And you..."** section that roasts the developer's habits. Read `.claude/stats.json` and analyse the `recentSessions` array (last 10 entries), `lifetime` stats, and `streak` data. Pull real numbers — no generic filler.
