...
- DOE KIT: Shown in the header row, right-aligned. If `~/doe-starter-kit` exists, run `git describe --tags --abbrev=0` to get the version. Quick-diff key syncable files (CLAUDE.md, ~/.claude/commands/*.md, .githooks/*, .claude/hooks/*.py) and count how many differ. If synced: show `DOE vX.Y.Z`. If unsynced: show `DOE vX.Y.Z (N pending)`. If the directory doesn't exist, omit the kit portion from the header.
